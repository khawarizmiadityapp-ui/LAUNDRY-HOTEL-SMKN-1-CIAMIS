<?php $__env->startSection('title', 'Laporan Pekerjaan Harian Petugas - Bening Laundry'); ?>
...
<div x-data="laporanPetugasApp()" x-cloak class="space-y-6 pb-12 animate-fade-in">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div class="flex flex-wrap items-center gap-2.5">
            <a href="<?php echo e(route('admin.laporan_petugas.pdf', request()->query())); ?>" 
            ...
            <a href="<?php echo e(route('admin.laporan_petugas.excel', request()->query())); ?>" 
    ...
    <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        ...
        <form method="GET" action="<?php echo e(route('admin.laporan_petugas.index')); ?>" id="filterForm" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3.5 items-end">
            ...
            <div>
                ...
                <select name="station" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                    <option value="all" <?php echo e(($selectedStation ?? 'all') == 'all' ? 'selected' : ''); ?>>Semua Stasiun</option>
                    <option value="washing" <?php echo e(($selectedStation ?? '') == 'washing' ? 'selected' : ''); ?>>🌊 Washing (Pencucian)</option>
                    <option value="ironing" <?php echo e(($selectedStation ?? '') == 'ironing' ? 'selected' : ''); ?>>♨️ Ironing (Penyetrikaan)</option>
                    <option value="packing" <?php echo e(($selectedStation ?? '') == 'packing' ? 'selected' : ''); ?>>📦 Packing (Pengemasan)</option>
                    <option value="kasir" <?php echo e(($selectedStation ?? '') == 'kasir' ? 'selected' : ''); ?>>💳 Kasir / POS</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="petugas" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                    <option value="all" <?php echo e(($selectedPetugas ?? 'all') == 'all' ? 'selected' : ''); ?>>Semua Petugas</option>
                    <?php $__currentLoopData = $petugasDropdown; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pName): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($pName); ?>" <?php echo e(($selectedPetugas ?? '') == $pName ? 'selected' : ''); ?>><?php echo e($pName); ?></option>
            ...
            <div>
                ...
                <select name="shift" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                    <option value="all" <?php echo e(($selectedShift ?? 'all') == 'all' ? 'selected' : ''); ?>>Semua Shift</option>
                    <option value="Pagi" <?php echo e(($selectedShift ?? '') == 'Pagi' ? 'selected' : ''); ?>>Pagi</option>
                    <option value="Siang" <?php echo e(($selectedShift ?? '') == 'Siang' ? 'selected' : ''); ?>>Siang</option>
                </select>
            </div>
            ...
            <div class="flex gap-2">
                ...
                <a href="<?php echo e(route('admin.laporan_petugas.index')); ?>" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-semibold rounded-xl transition flex items-center justify-center" title="Reset Filter">
                    ✕
                </a>
            </div>
        </form>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-800 mt-1"><?php echo e($stats['total_petugas']); ?></p>
                    <p class="text-xs text-emerald-600 font-semibold mt-1">
                        ✓ <?php echo e($stats['petugas_hadir']); ?> Hadir / Aktif
                    </p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-800 mt-1"><?php echo e($stats['total_tasks']); ?></p>
                    <p class="text-xs text-slate-500 font-medium mt-1">
                        Rata-rata: <?php echo e($stats['avg_tasks_per_petugas']); ?> per petugas
                    </p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-800 mt-1"><?php echo e(number_format($stats['total_weight'], 1)); ?> <span class="text-lg font-semibold text-slate-500">Kg</span></p>
                    <p class="text-xs text-slate-500 font-medium mt-1">
        ...
        <div class="bg-gradient-to-br from-slate-900 to-slate-800 p-5 rounded-2xl shadow-sm text-white">
            ...
            <div class="grid grid-cols-3 gap-2 mt-3 text-center">
                <div class="bg-white/10 rounded-xl p-2 backdrop-blur-sm">
                    ...
                    <span class="text-base font-black"><?php echo e($stats['washing_count']); ?></span>
                </div>
                <div class="bg-white/10 rounded-xl p-2 backdrop-blur-sm">
                    <span class="text-[10px] text-amber-300 font-bold block">♨️ Setrika</span>
                    <span class="text-base font-black"><?php echo e($stats['ironing_count']); ?></span>
                </div>
                <div class="bg-white/10 rounded-xl p-2 backdrop-blur-sm">
                    <span class="text-[10px] text-emerald-300 font-bold block">📦 Pack</span>
                    <span class="text-base font-black"><?php echo e($stats['packing_count']); ?></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="flex border-b border-slate-100 px-6 pt-4 gap-6 bg-slate-50/50">
            <button type="button" 
                ...
                <span class="ml-1 px-2 py-0.5 rounded-full text-[11px] bg-blue-100 text-blue-700 font-bold"><?php echo e($rekapPetugas->count()); ?></span>
            </button>
            ...
            <button type="button" 
                ...
                <span class="ml-1 px-2 py-0.5 rounded-full text-[11px] bg-slate-200 text-slate-700 font-bold"><?php echo e($taskLogs->count()); ?></span>
            </button>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs sm:text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $__empty_1 = true; $__currentLoopData = $rekapPetugas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $petugas): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="py-3.5 px-4 font-bold text-slate-400"><?php echo e($index + 1); ?></td>
                                <td class="py-3.5 px-4">
                                    <div class="font-bold text-slate-800"><?php echo e($petugas['nama']); ?></div>
                                    <span class="text-[11px] text-slate-400 font-mono">ID: <?php echo e($petugas['id_petugas']); ?></span>
                                </td>
                                <td class="py-3.5 px-4 text-center">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold bg-slate-100 text-slate-700">
                                        <?php echo e($petugas['shift']); ?>

                                    </span>
                                    <?php if($petugas['checked_in_at']): ?>
                                        <div class="text-[10px] text-emerald-600 font-medium mt-0.5">
                                            Hadir <?php echo e(\Carbon\Carbon::parse($petugas['checked_in_at'])->format('H:i')); ?>

                                        </div>
                                    <?php else: ?>
                                        <div class="text-[10px] text-slate-400 mt-0.5 capitalize"><?php echo e($petugas['status_kehadiran']); ?></div>
                                    <?php endif; ?>
                                </td>
                                <td class="py-3.5 px-4 text-center font-bold text-blue-600">
                                    <?php echo e($petugas['washing_count']); ?>

                                </td>
                                <td class="py-3.5 px-4 text-center font-bold text-amber-600">
                                    <?php echo e($petugas['ironing_count']); ?>

                                </td>
                                <td class="py-3.5 px-4 text-center font-bold text-purple-600">
                                    <?php echo e($petugas['packing_count']); ?>

                                </td>
                                <td class="py-3.5 px-4 text-center font-bold text-emerald-600">
                                    <?php echo e($petugas['kasir_count']); ?>

                                </td>
                                <td class="py-3.5 px-4 text-center">
                                    <span class="inline-flex items-center justify-center min-w-8 h-7 px-2 rounded-lg text-xs font-black <?php echo e($petugas['total_output'] > 0 ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-slate-100 text-slate-400'); ?>">
                                        <?php echo e($petugas['total_output']); ?>

                                    </span>
                                </td>
                                <td class="py-3.5 px-4 text-center font-semibold text-slate-700">
                                    <?php echo e(number_format($petugas['total_weight'], 1)); ?> kg
                                </td>
                                <td class="py-3.5 px-4 text-center">
                                    <button type="button" 
                                            @click="openDetailModal(<?php echo \Illuminate\Support\Js::from($petugas)->toHtml() ?>)"
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $taskLogs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="py-3.5 px-4 font-mono text-xs text-slate-600">
                                    <?php echo e($log['completed_at'] ? \Carbon\Carbon::parse($log['completed_at'])->translatedFormat('d M H:i') : '-'); ?>

                                </td>
                                <td class="py-3.5 px-4 font-bold text-slate-800">
                                    <?php echo e($log['petugas_name']); ?>
                                ...
                                <td class="py-3.5 px-4 font-mono font-bold text-blue-600">
                                    <?php echo e($log['transaksi_code']); ?>

                                </td>
                                <td class="py-3.5 px-4 font-medium text-slate-700">
                                    <?php echo e($log['customer_name']); ?>

                                </td>
                                <td class="py-3.5 px-4 text-xs text-slate-600">
                                    <span class="font-bold text-slate-800"><?php echo e($log['weight'] ? $log['weight'] . ' kg' : '-'); ?></span>
                                    <span class="text-slate-400 block"><?php echo e($log['layanan']); ?></span>
                                </td>
                                <td class="py-3.5 px-4 text-center">
...
function laporanPetugasApp() {
    return {
        filterMode: <?php echo \Illuminate\Support\Js::from($filterMode ?? 'harian')->toHtml() ?>,
        activeTab: 'rekap',
        singleDate: <?php echo \Illuminate\Support\Js::from($selectedDate ?? now()->format('Y-m-d'))->toHtml() ?>,
        dariDate: <?php echo \Illuminate\Support\Js::from($dari ?? now()->startOfMonth()->format('Y-m-d'))->toHtml() ?>,
        sampaiDate: <?php echo \Illuminate\Support\Js::from($sampai ?? now()->format('Y-m-d'))->toHtml() ?>,
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\LAUNDRY-HOTEL-SMKN-1-CIAMIS\resources\views/admin/laporan_petugas/index.blade.php ENDPATH**/ ?>