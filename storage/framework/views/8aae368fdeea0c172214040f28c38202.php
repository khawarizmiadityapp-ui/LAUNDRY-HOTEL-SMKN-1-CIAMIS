...
<script>
        ...
        petugasInputs.forEach(input => {
            const syncEvent = function (e) {
                ...
                petugasInputs.forEach(otherInput => {
                    if (otherInput !== e.target && otherInput.value !== newName) {
                        otherInput.value = newName;
                        // Memicu event agar Alpine JS mendeteksi perubahan state x-model
    ...
    document.addEventListener('submit', function (e) {
        if(e.target.tagName === 'FORM') {
            ...
            if(submitBtn && !submitBtn.disabled) {
                ...
                // Gunakan setTimeout untuk mencegah bug form batal submit di beberapa browser
                setTimeout(() => {
    ...
    window.petugasSearchComponent = function(petugasList, defaultName = '') {
        return {
            list: petugasList,
            ...
            init() {
                ...
                // Prioritize passed defaultName (from active station checkin) or localStorage
                if (defaultName) {
                    this.search = defaultName;
                    localStorage.setItem('petugas_piket_name', defaultName);
                    this.filterPetugas();
                } else {
                    const savedName = localStorage.getItem('petugas_piket_name');
                    if (savedName) {
            ...
            select(p) {
                ...
                let inputEl = this.$el.querySelector('input[name="petugas_name"]');
                if (inputEl) {
                    inputEl.value = p.nama; // Paksa update DOM secara sinkron untuk mencegah delay Alpine JS
                    inputEl.setCustomValidity('');
...
</html><?php /**PATH C:\laragon\www\LAUNDRY-HOTEL-SMKN-1-CIAMIS\resources\views/layouts/petugas_piket.blade.php ENDPATH**/ ?>