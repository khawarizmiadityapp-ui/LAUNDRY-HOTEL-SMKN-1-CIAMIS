<?php $__env->startSection('title', 'Inventaris - Staff Portal'); ?>
...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="px-4 py-2 bg-white border border-slate-100 rounded-xl shadow-sm">
                ...
                <p class="text-xl font-bold text-slate-900 leading-none"><?php echo e($inventory->count()); ?></p>
            </div>
            <div class="px-4 py-2 bg-white border border-slate-100 rounded-xl shadow-sm">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">Total Item</p>
                <p class="text-xl font-bold text-slate-900 leading-none"><?php echo e($inventory->flatten()->count()); ?></p>
            </div>
        </div>
    </div>
    ...
    <?php $__currentLoopData = $inventory; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category => $items): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="space-y-4">
        <!-- Category Header -->
        <div class="flex items-center gap-3">
            <div class="h-px flex-1 bg-slate-100"></div>
            <h2 class="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] whitespace-nowrap"><?php echo e($category); ?></h2>
            <div class="h-px flex-1 bg-slate-100"></div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            ...
            <div class="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-200 group">
                <div class="flex items-start justify-between mb-4">
                    <div class="w-12 h-12 rounded-xl <?php echo e($item->stock_units < $item->minimum_stock ? 'bg-rose-50 text-rose-500' : 'bg-indigo-50 text-indigo-600'); ?> flex items-center justify-center transition-colors">
                        <?php if(Str::contains(Str::lower($category), 'detergent')): ?>
                        ...
                        <?php elseif(Str::contains(Str::lower($category), 'fragrance')): ?>
                    ...
                    <?php if($item->stock_units < $item->minimum_stock): ?>
                ...
                <h3 class="font-bold text-slate-800 leading-tight"><?php echo e($item->name); ?></h3>
                <p class="text-xs text-slate-400 mt-1 uppercase tracking-wide"><?php echo e($item->type ?? 'Standard Supply'); ?></p>

                <div class="mt-6 flex items-end justify-between">
                    <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Jumlah Stok</p>
                        <?php if($item->unit_of_measurement === 'pcs' || $item->capacity_per_unit <= 1): ?>
                            <div class="flex items-baseline gap-1">
                                <span class="text-3xl font-black <?php echo e($item->stock_units < $item->minimum_stock ? 'text-rose-600' : 'text-slate-900'); ?>"><?php echo e($item->stock_units); ?></span>
                                <span class="text-xs font-bold text-slate-400"><?php echo e($item->unit_type); ?></span>
                            </div>
                        <?php else: ?>
                            <div class="flex flex-col">
                                ...
                                <span class="text-[11px] text-slate-500 font-semibold mt-1">
                                    + <?php echo e(number_format($item->stock_subunits)); ?> <?php echo e($item->unit_of_measurement); ?> aktif
                                </span>
                            </div>
                        <?php endif; ?>
                    </div>
                ...
                <div class="mt-5 pt-5 border-t border-slate-50 flex items-center justify-between">
                    <form action="<?php echo e(route('petugas_piket.inventory.adjust', $item->id)); ?>" method="POST" class="flex items-center gap-2">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="reason" value="Penyesuaian stok oleh petugas <?php echo e(auth()->user()->name); ?>">
                        ...
                        <span class="text-sm font-bold text-slate-700 min-w-[20px] text-center"><?php echo e($item->stock_units); ?></span>
                    ...
                    <div class="text-[10px] text-slate-300 italic">
                        Update: <?php echo e($item->updated_at->diffForHumans()); ?>
...
<div id="add-item-modal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 hidden flex items-center justify-center">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in mx-4">
        ...
        <form action="<?php echo e(route('petugas_piket.inventory.store')); ?>" method="POST" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
            <div>
...
<?php echo $__env->make('layouts.petugas_piket', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\LAUNDRY-HOTEL-SMKN-1-CIAMIS\resources\views/petugas_piket/inventory/index.blade.php ENDPATH**/ ?>