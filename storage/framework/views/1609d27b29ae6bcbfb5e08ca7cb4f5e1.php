                    ...
                    <table class="w-full text-left border-collapse min-w-[800px]">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100 text-[11px] uppercase tracking-wider text-slate-500 font-semibold">
                                ...
                                <th class="px-4 py-3 w-72">Form Petugas & Bahan</th>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group cursor-pointer" :class="{ 'bg-blue-50/50': selectedIds.includes(<?php echo e($trx->id); ?>) }" @click="const cb = $el.querySelector('input[type=checkbox]'); if(cb && $event.target.tagName !== 'INPUT' && $event.target.tagName !== 'BUTTON' && $event.target.tagName !== 'SELECT') { cb.click(); }">
                                ...
                                <td class="px-4 py-3 align-top">
                                    <div x-data="{ 
                                        ...
                                        <div class="mb-3" x-data="petugasSearchComponent(<?php echo e(json_encode($petugasList->map(fn($p) => ['nama' => $p->nama, 'id_petugas' => $p->id_petugas ?? $p->id]))); ?>, '<?php echo e($activePiketNama ?? ''); ?>')" @click.outside="showDropdown = false">
                                            <div class="relative">
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.petugas_piket', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\LAUNDRY-HOTEL-SMKN-1-CIAMIS\resources\views/petugas_piket/washing/index.blade.php ENDPATH**/ ?>