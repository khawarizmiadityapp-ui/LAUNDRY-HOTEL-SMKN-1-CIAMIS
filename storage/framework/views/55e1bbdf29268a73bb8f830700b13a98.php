...
<div x-data="posApp()" x-init="init()" class="flex flex-col lg:flex-row gap-0 -mx-5 lg:-mx-8 -my-8 <?php echo e(auth()->user()->role === 'admin' ? 'h-[calc(100vh-4rem)] lg:h-[calc(100vh-4rem)] overflow-hidden' : 'min-h-[calc(100vh-4rem)] lg:min-h-[calc(100vh-4rem)]'); ?>">
    ...
    <div class="flex-1 p-5 lg:p-7 overflow-y-auto">
            ...
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <button @click="viewMode = 'pickup'" 
                        ...
                        Siap Diambil
                        <?php if(count($readyToPickup) > 0): ?>
                        <span class="bg-rose-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full"><?php echo e(count($readyToPickup)); ?></span>
                        <?php endif; ?>
                    </button>
                </div>
        ...
        <div class="flex items-center gap-2 mb-5 flex-wrap">
            ...
            <button @click="activeCategory = '<?php echo e($kat); ?>'"
                    :class="activeCategory === '<?php echo e($kat); ?>' ? 'active' : ''"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            <div x-show="activeCategory === 'semua' || activeCategory === '<?php echo e($layanan->kategori); ?>'"
                 :class="isInCart(<?php echo e($layanan->id); ?>) ? 'selected' : ''"
                 class="pos-card bg-white border border-slate-100 rounded-2xl p-4 shadow-sm group">

                
                <?php if(auth()->user()->role === 'admin'): ?>
                <button @click.stop="openEditServiceModal(<?php echo e(json_encode($layanan)); ?>)"
                ...
                <div @click="toggleService(<?php echo e($layanan->id); ?>)">
                    ...
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center mb-3">
                        ...
                        <?php elseif($layanan->icon === 'hourglass'): ?>
                    ...
                    <h3 class="text-sm font-semibold text-slate-800 leading-tight mb-1"><?php echo e($layanan->nama); ?></h3>
                    <p class="text-[11px] text-slate-400 capitalize mb-2"><?php echo e($layanan->kategori); ?></p>



                    <p class="text-sm font-bold text-brand-600"><?php echo e($layanan->harga_format); ?><span class="text-[11px] font-normal text-slate-400"><?php echo e($layanan->satuan); ?></span></p>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[700px]">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white rounded-b-xl border border-t-0 border-slate-100">
                            <?php $__currentLoopData = $readyToPickup; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trx): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    <td class="px-6 py-4 align-top">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h3 class="font-bold text-slate-900 leading-tight"><?php echo e($trx->customer_name); ?></h3>
                                                <span class="inline-block mt-1 px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded-md uppercase"><?php echo e($trx->transaksi_code); ?></span>
                                                <p class="text-xs text-slate-500 mt-1"><?php echo e($trx->customer_phone); ?></p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 align-top">
                                        <p class="text-xs text-slate-500 mb-1.5 font-semibold"><?php echo e(count($trx->details)); ?> Item Layanan</p>
                                        <div class="flex flex-wrap gap-1">
                                            <?php $__currentLoopData = $trx->details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $det): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <span class="text-[10px] bg-brand-50 text-brand-600 px-2 py-1 rounded-md font-medium border border-brand-100"><?php echo e($det->layanan->nama); ?> (<?php echo e($det->qty); ?><?php echo e($det->layanan->satuan); ?>)</span>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 align-top">
                                        <p class="text-sm font-bold text-brand-600">Rp <?php echo e(number_format($trx->total_price, 0, ',', '.')); ?></p>
                                        <span class="inline-block mt-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full <?php echo e($trx->payment_status === 'lunas' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'); ?> uppercase tracking-wider">
                                            <?php echo e($trx->payment_status === 'lunas' ? 'LUNAS' : 'BELUM BAYAR'); ?>

                                        </span>
                                    </td>
                                    <td class="px-6 py-4 align-top">
                                        <form action="<?php echo e(route('pos.pickup', $trx->id)); ?>" method="POST">
                                            <?php echo csrf_field(); ?>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="number" name="dibayar" placeholder="Contoh: <?php echo e($trx->total_price); ?>" min="<?php echo e($trx->total_price); ?>" required
                                            ...
                                            <?php endif; ?>
                                            <button type="submit" 
                                                    onclick="return confirm('Konfirmasi pengambilan cucian untuk <?php echo e($trx->customer_name); ?>?')"
...
function posApp() {
    return {
        ...
        services: <?php echo json_encode($layanansJson, 15, 512) ?>,
        ...
        cashReceived: '',
        discountPercent: 0,
        petugasList: <?php echo json_encode($petugasList->map(fn($p) => ['nama' => $p->nama, 'id_petugas' => $p->id_petugas]), 512) ?>,
        kasirSearch: '',
        ...
        init() {
            ...
            if (!savedKasir && "<?php echo e(auth()->check()); ?>") {
                savedKasir = "<?php echo e(auth()->user()->name); ?>";
            }
            if (savedKasir) {
        ...
        async searchCustomers() {
            ...
            try {
                const res = await fetch(`<?php echo e(route('pos.customer.search')); ?>?q=${encodeURIComponent(this.customerSearch)}`);
        ...
        async saveNewCustomer() {
            try {
                const res = await fetch('<?php echo e(route("pos.customer.store")); ?>', {
                    method: 'POST',
                    headers: {
        ...
        async submitOrder() {
            ...
            try {
                const res = await fetch('<?php echo e(route("pos.order.store")); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
        ...
        async saveService() {
            ...
            try {
                const url = this.serviceForm.id
                    ...
                    : '<?php echo e(route("admin.layanan.store")); ?>';
...
<?php echo $__env->make(auth()->user()->role === 'admin' ? 'layouts.admin' : 'layouts.petugas_piket', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\LAUNDRY-HOTEL-SMKN-1-CIAMIS\resources\views/pos/index.blade.php ENDPATH**/ ?>