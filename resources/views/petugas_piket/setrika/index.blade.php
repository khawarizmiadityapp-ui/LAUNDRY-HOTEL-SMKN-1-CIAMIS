...
<div class="p-6 max-w-5xl mx-auto animate-fade-in">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100">
        ...
        <div class="p-6" x-data="{ 
            selectedIds: [], 
            allSelected: false,
            transactionIds: {{ json_encode($transactions->pluck('id')) }},
            toggleAll() {
                if (this.allSelected) {
                    ...
                    <table class="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($transactions as $trx)
                                <tr class="hover:bg-slate-50/50 transition-colors cursor-pointer" :class="{ 'bg-blue-50/50': selectedIds.includes({{ $trx->id }}) }" @click="const cb = $el.querySelector('input[type=checkbox]'); if(cb && $event.target.tagName !== 'INPUT' && $event.target.tagName !== 'BUTTON' && $event.target.tagName !== 'SELECT') { cb.click(); }">
            ...
            <div x-show="selectedIds.length > 0" x-transition.opacity
                ...
                <form action="{{ route('petugas_piket.tasks.completeBulk') }}" method="POST" class="flex items-center gap-2">
                    @csrf
                    <input type="hidden" name="stage" value="ironing">
