...
<head>
    ...
    <title>Daftar Customer - Bening Laundry</title>
        ...
        <div class="relative hidden md:flex flex-col justify-between p-12 bg-slate-900 text-white overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <h1 class="text-4xl font-900 leading-tight tracking-tight mt-8">
                    Bergabung.<br>
                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-blue-400">Dan Nikmati Kemudahan.</span>
                </h1>
                <p class="mt-4 text-slate-300 font-medium leading-relaxed max-w-sm">
                    Buat akun pribadi Anda untuk mulai memesan layanan laundry premium dengan satu ketukan.
                </p>
            </div>
        </div>

        {{-- Right Side: Register Form --}}
        <div class="p-8 md:p-12 lg:p-16 flex flex-col justify-center bg-white relative max-h-[90vh] overflow-y-auto">
            ...
            <div class="space-y-3 mb-8">
                <h2 class="text-3xl font-900 tracking-tight text-slate-900">Buat Akun Anda</h2>
                <p class="text-slate-500 font-medium text-sm">Silakan lengkapi formulir di bawah untuk mendaftar akun pelanggan baru.</p>
            </div>
            ...
            <form action="{{ route('customer.register.post') }}" method="POST" class="space-y-5">
                ...
                {{-- Nama --}}
                <div class="space-y-2">
                    <label class="text-[11px] text-slate-500 font-bold uppercase tracking-widest ml-1">Nama Lengkap</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                             <i class="fa-solid fa-user text-slate-400"></i>
                        </div>
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Budi Santoso" class="block w-full pl-14 pr-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl text-slate-700 font-semibold placeholder-slate-300 focus:bg-white focus:border-sky-500/30 focus:ring-0 focus:outline-none transition-all">
                    </div>
                    @error('name')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="password" name="password" required placeholder="Minimal 8 karakter" class="block w-full pl-14 pr-4 py-4 bg-slate-50 border-2 border-transparent rounded-2xl text-slate-700 font-semibold placeholder-slate-300 focus:bg-white focus:border-sky-500/30 focus:ring-0 focus:outline-none transition-all">
                        </div>
                        @error('password')
                            <p class="text-red-500 text-xs font-semibold mt-1 ml-1">{{ $message }}</p>
                        @enderror
                    ...
                    {{-- Confirm Password --}}
                    <div class="space-y-2">
                        <label class="text-[11px] text-slate-500 font-bold uppercase tracking-widest ml-1">Ulangi Sandi</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                 <i class="fa-solid fa-check text-slate-400"></i>
                            </div>
                            <input type="password" name="password_confirmation" required placeholder="Ulangi kembali" class="block w-full pl-14 pr-4 py-4 bg-slate-50 border-2 border-transparent rounded-2xl text-slate-700 font-semibold placeholder-slate-300 focus:bg-white focus:border-sky-500/30 focus:ring-0 focus:outline-none transition-all">
                        </div>
                    </div>
                </div>

                <button type="submit" class="w-full py-5 bg-slate-900 text-white rounded-2xl font-bold shadow-xl shadow-slate-900/25 hover:bg-slate-800 hover:shadow-2xl hover:shadow-slate-900/40 hover:-translate-y-0.5 transition-all text-sm uppercase tracking-widest mt-4">
                    Buat Akun Anda
                </button>
            </form>
            ...
            <div class="mt-8 text-center bg-slate-50 rounded-2xl p-6 border border-slate-100">
                <p class="text-sm font-medium text-slate-600">
                    ...
                    <a href="{{ route('customer.login') }}" class="text-sky-500 font-bold hover:underline transition-all">Masuk</a>
                </p>
            </div>
