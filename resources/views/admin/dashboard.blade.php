...
<div class="bg-white rounded-2xl shadow-card animate-fade-up" style="animation-delay:.2s">
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
        ...
        <a href="{{ route('admin.transactions.index') }}" class="text-xs font-semibold text-brand-600 hover:text-brand-700 flex items-center gap-1 transition">
            Lihat Semua
            ...
            <tbody class="divide-y divide-slate-50">
                @foreach($recentTransactions as $index => $trx)
                    <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                        <td class="px-6 py-3.5">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="leading-none">
                                    <p class="font-medium text-slate-800">{{ $trx->customer_name }}</p>
                                    <p class="text-xs text-slate-400 mt-0.5">ID: {{ $trx->transaksi_code }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3.5 text-slate-600">
                            {{ ucfirst($trx->service_type) }}
                        </td>
                        ...
                        <td class="px-4 py-3.5 text-right font-semibold text-slate-800">Rp {{ number_format($trx->total_price, 0, ',', '.') }}</td>
                        <td class="px-6 py-3.5 text-right text-xs text-slate-400 whitespace-nowrap">{{ $trx->created_at->format('M d, H:i') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="border-t border-slate-100 px-6 py-4 flex justify-center">
        <a href="{{ route('admin.transactions.index') }}" class="flex items-center gap-2 text-sm font-semibold text-brand-600 hover:text-brand-700 transition px-4 py-2 rounded-xl hover:bg-brand-50">
